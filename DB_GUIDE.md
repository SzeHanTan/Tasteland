# 🗄️ Database & API Guide (Supabase + Retrofit)
...
The singleton "Connection Manager." Use this to get an instance of the API.

**`SupabaseAPI`**

An Interface listing all available server commands (Get items, Login, Update Profile).

**`SessionManager`**

Handles saving/loading the User's **Access Token** so they stay logged in.

**`ShoppingItem`**

Data model for the shopping list (matches `shopping_items` table).

**`UserProfile`**

Data model for the user profile (matches `profiles` table).
